...
class User(Base):
    ...
    grades: Mapped[list["Grade"]] = relationship("Grade", foreign_keys="Grade.user_id", back_populates="user")
...
class Course(Base):
    ...
    enrollment_code: Mapped[str] = mapped_column(String(16), unique=True, nullable=False, default=lambda: secrets.token_urlsafe(6))
    ...
    exercises: Mapped[list["Exercise"]] = relationship("Exercise", back_populates="course", cascade="all, delete-orphan", order_by="Exercise.order_index")
...
class Exercise(Base):
    ...
    allowed_extensions: Mapped[str] = mapped_column(String(255), nullable=False, default="pdf")
    allow_late_upload: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    grade_type: Mapped[str] = mapped_column(String(20), nullable=False, default="numeric")  # numeric | pass_fail
    ...
    upload_requirement: Mapped[str] = mapped_column(String(20), nullable=False, default="optional")
    url_requirement: Mapped[str] = mapped_column(String(20), nullable=False, default="optional")
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    ...
    grades: Mapped[list["Grade"]] = relationship("Grade", back_populates="exercise", cascade="all, delete-orphan")
    finalizations: Mapped[list["Finalization"]] = relationship("Finalization", back_populates="exercise", cascade="all, delete-orphan")
...
class Submission(Base):
    ...
    submission_type: Mapped[str] = mapped_column(String(10), nullable=False)  # file | url
...
class Finalization(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False)
    exercise_id: Mapped[int] = mapped_column(Integer, ForeignKey("exercises.id"), nullable=False)
    finalized_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), nullable=False)

    user: Mapped["User"] = relationship("User")
    exercise: Mapped["Exercise"] = relationship("Exercise", back_populates="finalizations")
